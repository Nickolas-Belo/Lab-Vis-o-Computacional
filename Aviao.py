...
img_aviao = cv2.imread('./aviao.jpg')
img_aviao = cv2.cvtColor(img_aviao, cv2.COLOR_BGR2RGB)

#Convertendo para preto e branco (RGB -> Gray Scale -> BW)
img_gray = cv2.cvtColor(img_aviao, cv2.COLOR_RGB2GRAY)
a = img_gray.max()
...
img_copy = img_aviao.copy()
final = cv2.drawContours(img_copy, contours, contourIdx = -1,
                         color = (255, 0, 0), thickness = 2)


#plot imagens
imagens = [img_aviao,img_blur,img_gray,edges_gray,edges_blur,edges_gray,final]
