...
img_castelo = cv2.imread('./castelo.jpeg')
img_castelo = cv2.cvtColor(img_castelo, cv2.COLOR_BGR2RGB)

#Convertendo para preto e branco (RGB -> Gray Scale -> BW)
img_gray = cv2.cvtColor(img_castelo, cv2.COLOR_RGB2GRAY)
a = img_gray.max()
...
edges_blur = cv2.Canny(image=img_blur, threshold1=a/1.6, threshold2=a/1.7)
